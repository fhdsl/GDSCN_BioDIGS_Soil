...
dataset %>%
    ...
    summarize(new_statistical_measure = mean(column_name))
    
```{r, message = FALSE, warning = FALSE}

soil.values.clean %>%
    group_by(region) %>%
    summarize(Regional_mean = mean(As_EPA3051))
```
...
vector_name <- c("column_name1", "column_name2")

```{r}
columns_to_include <- c("As_EPA3051", "Cd_EPA3051", "Cr_EPA3051", "Cu_EPA3051", "Ni_EPA3051", "Pb_EPA3051", "Zn_EPA3051")
```
...
9. Calculate both the mean and maximum values for concentrations that were determined using the Mehlich3 test. (HINT: change the )
